...
> **2026-05-16 视觉润色更新**：行采用「左侧竖向紧迫度彩条 + SF Symbol 图标 + 右侧彩色胶囊倒计时」；emoji 图标改 SF Symbols；空/错误态用 SF Symbol + 渐变；底栏改图标+文字纵向按钮；黄色诊断条移出主界面，改到「设置 → 关于 → 排查工具」。紧迫度配色：逾期=红，≤1天=橙，≤3天=黄，>3天=绿，无期限=灰。
>
> **「按学科」改为可折叠手风琴**：默认全部收起，每门课一行（chevron + 课名 + 收起时的紧迫度小圆点 + 数量胶囊），点击展开/收起该课，解决课多时需滚动到底的问题。`expandedCourses: Set<String>` 维护展开态。「全部」视图仍为按时间平铺。
>
> 下方为原始规范，细节以最新代码为准。
